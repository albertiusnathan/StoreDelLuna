...
function execCategorizer() {
  var albumsCb = document.querySelectorAll(".albumChecks input[type='checkbox']");
  var filters = {
    albums: getClassOfCheckedCheckboxes(albumsCb)
  };
...
function filterResults(filters) {
  ...
  for (var i = 0; i < rElems.length; i++) {
    ...
    if (filters.albums.length > 0) {
      var isHidden = true;
      for (var j = 0; j < filters.albums.length; j++) {
        var filter = filters.albums[j];
